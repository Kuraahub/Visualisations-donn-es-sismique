...
function plotPieChart(data) {
    const statusCounts = data.map(earthquake => earthquake.status);
    ...
    const pieData = [{
        values: [automaticCount, reviewedCount],
        labels: ['Automatic', 'Reviewed'],
        type: 'pie'
    }];

    const layout = {
        title: 'Proportion of Earthquakes - Automatic vs Reviewed'
    };
